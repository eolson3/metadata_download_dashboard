#!/usr/bin/env python3
"""Run the v0.10 local OSF metadata, wiki-history, and activity-log checklist.

The pilot uses only the Python standard library. It keeps the OSF Personal
...
SCRIPT_VERSION = "0.10-no-empty-summary-folders"
PAGE_SIZE = 100
REQUEST_TIMEOUT = 90
MAX_RETRIES = 6
...
ACTION_FOLDER_PARTS = {
    ...
    "metadata_archive": ("Full Metadata Archive",),
...
SUMMARY_METADATA_RELATIONSHIPS = {
    "contributors",
...
def group_export_issues(issues: list[ExportIssue]) -> list[dict[str, Any]]:
    ...
    for (severity, element_type, reason), members in grouped.items():
        projects = {
            (issue.project_guid, issue.project_title)
        ...
        output.append(
            {
                ...
                "projects": [
                    ...
                    for guid, title in sorted(projects, key=lambda item: (item[1].casefold(), item[0]))
                ],
                "element_ids": [
                    issue.element_id
                    for issue in members
...
def issue_content_type(issue: ExportIssue) -> str | None:
    ...
    if "activity-log" in element_type:
        return "logs"
    if "metadata summary" in element_type:
...
def build_retry_plan(
    ...
    public_scopes = {
        content_type: sorted(guids)
        for content_type, guids in sorted(scopes.items())
    }
    scope_parts = [
...
def ensure_hierarchy(root: NodeRecord, root_folder: Path, action: str) -> None:
    def create(node: NodeRecord, folder: Path) -> None:
        ...
        if action != "metadata_summary" or node.guid == root.guid:
...
<title>{html.escape(title)} [{html.escape(guid)}] - Full Metadata Archive</title>
<style>
...
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1">
<meta http-equiv="Content-Security-Policy" content="default-src 'none'; style-src 'unsafe-inline';">
<title>{html.escape(title)} [{html.escape(guid)}] - Metadata Summary Catalog</title>
<style>
...
<header><h1>{html.escape(title)} [{html.escape(guid)}]</h1><p>Metadata summary catalog · project and component records</p></header>
<main>
...
def write_descriptive_metadata_catalog(
    ...
    catalog = {
        "format": "OSF DataCite-inspired metadata summary catalog",
        "format_version": "1.0",
        "based_on": {
        ...
        "source_issues": [issue.public_dict() for issue in issues],
    }
    stem = f"{root.display_name} - Metadata Summary Catalog"
    summary_folder = action_output_folder(root, "metadata_summary")
    json_path = summary_folder / f"{stem}.json"
    html_path = summary_folder / f"{stem}.html"
...
def collect_metadata_package(
    ...
    *,
    ...
    try:
        ...
        add_issue(issues, node, f"{issue_prefix}custom metadata record", node.guid, error)
    ...
    if include_cedar:
        try:
            ...
            add_issue(issues, node, f"{issue_prefix}CEDAR metadata record", node.guid, error)
    ...
    return {
        "format": (
            "OSF full metadata archive"
            if relationship_keys is None and include_cedar
            else "OSF metadata summary source record"
        ),
        "format_version": "1.0",
        "exported_utc": utc_now(),
        "exporter_version": SCRIPT_VERSION,
...
def complete_metadata_paths(node: NodeRecord) -> tuple[Path, Path]:
    stem = owner_prefixed_name(node, " - Full Metadata Archive")
    metadata_folder = action_output_folder(node, "metadata_archive")
...
def export_metadata(
    ...
    if mode not in {"summary", "archive"}:
        raise PilotError(f"Unknown metadata export mode: {mode}")
    ...
    for index, node in enumerate(nodes):
        ...
        report(
            ...
            f"{'Full metadata archive' if archive else 'Metadata summary'}: {node.display_name}",
        )
        try:
            ...
            package = collect_metadata_package(
                ...
                job,
                issues,
                relationship_keys=None if archive else SUMMARY_METADATA_RELATIONSHIPS,
                include_cedar=archive,
                issue_prefix="" if archive else "metadata summary ",
            )
            ...
            add_issue(
                issues,
                node,
                "core metadata record" if archive else "metadata summary core record",
                node.guid,
                error,
                critical=True,
            )

    aggregate_nodes = all_nodes if archive and all_nodes is not None else nodes
    target_guids = {node.guid for node in nodes}
    for node in aggregate_nodes:
        package = refreshed.get(node.guid)
        if package is None and archive:
        ...
        if not isinstance(raw, dict):
            add_issue(
                issues,
                node,
                "existing metadata package" if archive else "metadata summary core record",
                node.guid,
        ...
        rows.append(metadata_csv_row(node, raw, root.guid, package))
        resources.append(package)

    if archive:
        report(start + span, "Full metadata archive finished")
        return len(resources), sum(
            node.related_metadata_count
            for node in aggregate_nodes
            if node.metadata_count
        )

    assert root.folder is not None
    report(start + span * 0.97, "Writing metadata summary catalog")
    try:
        write_descriptive_metadata_catalog(root, resources, issues)
    except (OSError, TypeError, ValueError) as error:
        add_issue(
            issues,
            root,
            "metadata summary catalog",
            root.guid,
            error,
            critical=True,
        )

    stem = f"{root.display_name} - Project and Component Metadata Summary"
    fields = [
        "guid",
        "title",
        "url",
    ...
    try:
        summary_folder = action_output_folder(root, "metadata_summary")
        with (summary_folder / f"{stem}.csv").open(
        ...
        with (summary_folder / f"{stem}.json").open(
    ...
    report(start + span, "Metadata summary finished")
    return len(resources), sum(
        node.related_metadata_count
        for node in aggregate_nodes
        if node.metadata_count
...
def export_wikis(
    ...
    *,
    ...
    if not include_current and not include_history:
        raise PilotError("Choose current wikis, wiki version history, or both.")
    page_total = 0
    version_total = 0
    node_count = max(1, len(nodes))
    ...
    report(
        start + span,
        "Current wiki export finished" if include_current else "Wiki version-history export finished",
    )
...
def organize_logs(
    ...
    full_node_map = {
        node.guid: node
        for node in (all_nodes if all_nodes is not None else nodes)
    }
...
def write_export_summary(
    ...
    assert root.folder is not None
    summary_label = (
        f"{action_title(job.action)} {job.retry_mode.title()} Retry "
        f"[{job.id}] Summary"
        if job.retry_of
    ...
    notes_by_action = {
        "metadata_summary": [
            "The root project's Metadata Summary folder contains a DataCite-inspired catalog in HTML and JSON.",
            "That folder also contains combined project/component metadata summaries in expanded CSV and JSON.",
            "The summary includes core descriptive fields, contributors, institutions, identifiers, license, language, resource type, and funding when available.",
        ],
        "metadata_archive": [
            "Each Full Metadata Archive folder contains readable HTML, comprehensive JSON, and normalized API source records.",
            "The full archive includes custom and CEDAR metadata plus supported relationship records.",
    ...
    document = {
        ...
        "notes": [
            *notes_by_action[job.action],
            "Project files and ZIP archives are not created by this version.",
        ],
    }
    with path.open("w", encoding="utf-8", newline="\n") as handle:
        json.dump(document, handle, ensure_ascii=False, indent=2)
    ...
    def run_job(self, job: ExportJob) -> None:
        ...
        try:
            ...
            if not content_types:
                raise PilotError("The retry did not contain any recoverable export elements.")
            node_map = {node.guid: node for node in nodes}
            ...
            for task_index, content_type in enumerate(content_types):
                ...
                if content_type in {"metadata_summary", "metadata_archive"}:
                    metadata_records, related_records = export_metadata(
                        ...
                        task_start,
                        task_span,
                        mode="summary" if content_type == "metadata_summary" else "archive",
                        all_nodes=nodes,
                    )
                    ...
                    self.report(job, task_start + task_span * 0.86, "Organizing activity logs")
            ...
            critical_count = sum(1 for issue in issues if issue.severity == "critical")
            omission_count = sum(1 for issue in issues if issue.severity == "omission")
            affected_count = len({issue.project_guid for issue in issues if issue.project_guid})
            with self.lock:
                job.issues = issues
                if critical_count:
                    job.status = "failed"
...
.batch-panel,.panel,.jobs{padding:17px;margin-bottom:15px}.batch-panel h2,.panel h2,.jobs h2{margin:0 0 10px}.batch-buttons{display:flex;flex-wrap:wrap;gap:8px;margin:11px 0}.options{display:flex;flex-wrap:wrap;gap:18px;color:var(--muted);font-size:.88rem}.options label,.batch-label{display:inline-flex;align-items:center;gap:5px}.export-choices{display:grid;grid-template-columns:repeat(auto-fit,minmax(230px,1fr));gap:8px;margin:13px 0}.export-choice{display:flex;align-items:flex-start;gap:8px;padding:10px;border:1px solid var(--line);border-radius:7px;background:#f8fafb}.export-choice input{margin-top:3px}.export-choice strong,.export-choice small{display:block}.export-choice small{margin-top:2px;color:var(--muted)}
...
.row-actions{display:flex;flex-wrap:wrap;gap:6px;margin-top:8px}.action-button{padding:5px 8px;font-size:.78rem;color:var(--blue2)}
...
.job{padding:12px 0;border-top:1px solid var(--line)}.job:first-of-type{border-top:0}.job-head,.job-foot{display:flex;flex-wrap:wrap;align-items:center;justify-content:space-between;gap:8px}.job-title{font-weight:750}.job-status{padding:2px 7px;border-radius:999px;background:#edf2f5;font-size:.72rem;font-weight:800}.job-status.completed{color:#276137;background:var(--green)}.job-status.failed,.job-status.cancelled{color:var(--red);background:#f9e5e3}.job-status.completed_with_omissions{color:#814c0b;background:var(--orange)}.job-progress{margin:8px 0}.job-message{color:var(--muted);font-size:.82rem}.job-issues{margin:7px 0;font-size:.78rem}.job-issues summary{cursor:pointer;font-weight:750}.job-issues>ul>li{margin:7px 0}.job-issues li.critical{color:var(--red)}.job-issues li.omission{color:#814c0b}.issue-elements{margin:4px 0 7px;padding-left:22px;color:var(--ink)}.issue-elements li{margin:2px 0}.retry-note{margin:5px 0;color:var(--muted);font-size:.78rem}.empty{color:var(--muted);font-style:italic}
...
<body>
<header><h1>OSF Export Checklist</h1><p>__ACCOUNT__ · selectable export edition · <strong>__VERSION__</strong></p></header>
<main>
  ...
  <section class="batch-panel">
    <h2>Batch export</h2><p class="muted">Select “Include in batch” on one or more top-level projects, check the content to export, then run the batch.</p>
    ...
    <div class="export-choices">
      <label class="export-choice"><input type="checkbox" name="batchAction" value="metadata_summary"><span><strong>Metadata summary</strong><small>DataCite-inspired catalog plus combined CSV and JSON of critical project/component metadata.</small></span></label>
      <label class="export-choice"><input type="checkbox" name="batchAction" value="metadata_archive"><span><strong>Full metadata archive</strong><small>Per-item comprehensive HTML/JSON, normalized API responses, CEDAR/custom metadata, and resolved relationships.</small></span></label>
    ...
    <div class="options"><span>Each checked content type runs independently and writes to its own named folder inside the matching project/component tree. An omission in one does not require rerunning the others. Project files, Storage Usage metadata, and ZIP creation remain excluded.</span></div>
...
async function queueJobs(guids,action){if(!guids.length){alert('Select at least one project for the batch export.');return}try{await api('/api/jobs',{guids,action});await poll()}catch(error){alert(error.message)}}
...
document.getElementById('runBatch').onclick=async()=>{
  const guids=[...document.querySelectorAll('.node[data-root=true]')].filter(node=>node.querySelector(':scope > .node-row .batch-check').checked).map(node=>node.dataset.guid);
  const actions=[...document.querySelectorAll('input[name=batchAction]:checked')].map(input=>input.value);
  if(!guids.length){alert('Select at least one project for the batch export.');return}
  if(!actions.length){alert('Check at least one export content option.');return}
  try{await api('/api/jobs',{guids,actions});await poll()}catch(error){alert(error.message)}
};
...
function renderJobs(jobs){
  ...
  const openIssues=new Set([...area.querySelectorAll('.job-issues[open]')].map(details=>details.dataset.job));
  const statusLabels={completed:'Completed',completed_with_omissions:'Completed with omissions',failed:'Failed',cancelled:'Cancelled',queued:'Queued',running:'Running'};
  const actionLabels={metadata_summary:'Metadata summary',metadata_archive:'Full metadata archive',wiki_current:'Current wikis',wiki_history:'Wiki version history',logs:'Activity logs'};
  if(!jobs.length){area.innerHTML='<div class="empty">No exports queued yet.</div>';return}
  area.innerHTML=jobs.map(job=>{
    const active=['queued','running'].includes(job.status);
    const retry=['failed','cancelled','completed_with_omissions'].includes(job.status);
    const groups=job.issue_groups||[];
    const issues=groups.length?`<details class="job-issues" data-job="${escapeText(job.id)}"${openIssues.has(job.id)?' open':''}><summary>${job.critical_failure_count||0} critical failure(s), ${job.omission_count||0} omission(s)</summary><ul>${groups.map(group=>`<li class="${escapeText(group.severity)}"><strong>${group.severity==='critical'?'Critical':'Omission'}:</strong> ${escapeText(group.message)}<ul class="issue-elements">${(group.elements||[]).map(element=>`<li>${escapeText(element.description)}</li>`).join('')}</ul></li>`).join('')}</ul></details>`:'';
    const plan=job.retry_plan||{};
    const retryLabel=plan.mode==='targeted'?'Retry affected items':'Retry full action';
    const retryNote=retry&&plan.description?`<div class="retry-note">${escapeText(plan.description)}</div>`:'';
    const retryKind=job.retry_of?` · ${escapeText(job.retry_mode)} retry`:'';
  ...
  area.querySelectorAll('[data-cancel]').forEach(button=>button.onclick=()=>api(`/api/jobs/${button.dataset.cancel}/cancel`,{}).then(poll).catch(error=>alert(error.message)));
  area.querySelectorAll('[data-retry]').forEach(button=>button.onclick=()=>api(`/api/jobs/${button.dataset.retry}/retry`,{}).then(poll).catch(error=>alert(error.message)));
  area.querySelectorAll('[data-open]').forEach(button=>button.onclick=()=>api('/api/open-folder',{job_id:button.dataset.open}).catch(error=>alert(error.message)))
}
        ...
        try:
            ...
            if path == "/api/jobs":
                ...
                if not isinstance(raw_actions, list) or not raw_actions:
                    raise PilotError("Choose at least one export content option.")
                actions = list(
                    dict.fromkeys(str(action or "") for action in raw_actions)
                )
                invalid_actions = [
                    action for action in actions if action not in VALID_ACTIONS
                ]
                if invalid_actions:
                    raise PilotError(f"Unknown export action: {invalid_actions[0]}")
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Open a local OSF checklist with independently selectable metadata, wiki, and activity-log exports."
    )
    parser.add_argument(
        "--limit",
        type=int,
    ...
    parser.add_argument(
        "--version",
        action="version",
        version=f"%(prog)s {SCRIPT_VERSION} (selectable metadata summaries, full metadata archives, current wikis, wiki histories, and activity logs; project files disabled)",
    )
